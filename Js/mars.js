...
(() => {
  ...
  const NASA_KEY = "WWKOCbgjadMLteoeOBcczsNtdPt1SHrDaa9haGCb";
  ...
  function renderSkeleton() {
    gallery.innerHTML = Array.from({ length: 8 })
      .map(
        () =>
          `<div class="mars-photo glass skeleton-card"><div class="skeleton skeleton-media"></div></div>`
      )
      .join("");
  }
  ...
  function cardTemplate(photo) {
    return `
      <figure class="mars-photo glass" data-src="${
        photo.img_src
      }" data-alt="${escapeHtml(photo.rover.name)} — ${escapeHtml(
      photo.camera.full_name
    )}">
        <img src="${photo.img_src}" alt="${escapeHtml(
      photo.rover.name
    )} rover photo from ${escapeHtml(photo.camera.full_name)}" loading="lazy" />
